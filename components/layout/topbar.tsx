...
export default function Topbar() {
  ...
  // 🧑‍💻 ক্লায়েন্ট সাইডে লোড হওয়ার পর localStorage থেকে মার্চেন্ট আইডি নেওয়া হচ্ছে
  useEffect(() => {
    if (typeof window !== "undefined") {
      const storedMerchant = localStorage.getItem("merchant");
      if (storedMerchant) {
  ...
  // 🔄 মার্চেন্ট আইডি পাওয়ার পর এপিআই থেকে স্টোরগুলো তুলে আনা হচ্ছে
  const { data: stores = [], isLoading } = useGetMyStores(merchantId);
  ...
  return (
    <header className="w-full h-[72px] bg-white border-b border-[#E9E9E9] px-4 flex items-center justify-center shrink-0 shadow-sm">
      {/* Container */}
      <div className="w-full flex items-center justify-between gap-4">
        
        {/* Left Side: Mobile Hamburger & Page Title */}
        <div className="flex items-center gap-4 min-w-0">
          {/* 🍔 মোবাইল হ্যামবার্গার বাটন (যা ছোট স্ক্রিনে সাইডবার ওপেন করবে) */}
          <Link
            href="?sidebar=open"
            className="lg:hidden p-1.5 rounded-lg border border-[#E9E9E9] hover:bg-gray-50 text-[#0E2038] transition shrink-0"
          >
          ...
          {/* ড্যাশবোর্ড টাইটেল */}
          <h1 className="h6-bold truncate select-none text-[#0E2038]">Dashboard</h1>
        </div>

        {/* Right Side Section: Modular Sub-components & Dynamic Select */}
        <div className="flex items-center gap-2 sm:gap-3 shrink-0">
          
          {/* 🏪 ডাইনামিক স্টোর সিলেকশন ড্রপডাউন */}
          {isLoading || !merchantId ? (
            <div className="text-xs text-gray-400 bg-gray-50 px-3 py-2 rounded-xl border animate-pulse">
              Loading Stores...
            </div>
            ...
            <select
              value={currentStoreId || ""}
              onChange={(e) => setCurrentStoreId(e.target.value)}
              className="bg-gray-50 border border-[#E9E9E9] text-[#0E2038] text-xs font-bold rounded-xl p-2.5 outline-none focus:border-[#F74608] min-w-[140px] max-w-[180px] cursor-pointer truncate"
            >
              {stores.map((store) => (
                <option key={store.id} value={store.id}>
                  🏪 {store.name}
